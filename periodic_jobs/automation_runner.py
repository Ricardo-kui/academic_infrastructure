...
def stage_observer(date_str: str | None = None, dry_run: bool = False) -> int:
...
def run_daily(date_str: str | None = None, dry_run: bool = False) -> int:
    """Daily pipeline: observer → consolidate → dashboard."""
    print("=" * 60)
    print(f"[*] Daily Pipeline — {date_str or datetime.now().strftime('%Y-%m-%d')}")
    print("=" * 60)

    codes = []
    codes.append(stage_observer(date_str, dry_run))
    codes.append(stage_consolidate(dry_run))
    codes.append(stage_dashboard())
...
def run_agentic(date_str: str | None = None, dry_run: bool = False, force: bool = False) -> int:
    """Agentic daily pipeline: Kimi observer → consolidate → dashboard."""
    print("=" * 60)
    print(f"[*] Agentic Daily Pipeline — {date_str or datetime.now().strftime('%Y-%m-%d')}")
    print("=" * 60)

    codes = []
    codes.append(stage_agentic_observer(date_str, dry_run, force))
    codes.append(stage_consolidate(dry_run))
    codes.append(stage_dashboard())
...
def main() -> int:
    ...
    parser.add_argument("--force", action="store_true", help="Force overwrite (agentic mode only)")
    args = parser.parse_args()

    if args.mode == "daily":
        return run_daily(args.date, args.dry_run)
